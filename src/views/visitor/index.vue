<template>
	<div class="bind_rool">
		<div class="br_block visitor">
			...
			<van-cell-group>
				<!-- <van-field v-model="JSON.parse($route.query.para).address||''" required label="访问地址" disabled/> -->
...
<script>
	...
	export default {
		data() {
			return {
				...
				form: {
					...
					img: '', //图片
					reason: '' ,//访问理由
					// id:JSON.parse(this.$route.query.para).id
				},
				key: '',
				valueTime: new Date(),
				fileList: [],
		...
		mounted() {
			// let novTime = Date.parse(new Date()).toString()
			// if(JSON.parse(this.$route.query.para).exp_time < novTime.substr(0,10)){//过期时间小于现在时间
			// 	this.chaoshi = true
			// }
		...
		methods: {
			...
			// 	this.$post('NewModer&a=uploadImg', params, 'formdata').then(r => {
			// 		this.form.img = r.msg
			// 		this.imgshow = false;

  ...
  methods: {
    ...
    getData() { // 获取楼栋
      this.$post('HouseApi&a=village_single_list', {
		...
		this.setForm('address',this.village_info.village_name)
		this.setForm('village_id',this.village_info.village_id)
    ...
    getDanYuan(item) { // 获取单元
      this.$post('HouseApi&a=village_unit_list', {
		...
		this.setForm('address',item.single_name)
		this.setForm('single_id',item.single_id)
    ...
    getFloor(item) { // 获取楼层
      this.$post('HouseApi&a=get_village_layer_list', {
        ...
        floor_id: item.floor_id
		  ...
		  this.setForm('address',item.floor_name)
		  this.setForm('floor_id',item.floor_id)
    ...
    getRoom(item) { // 获取房间
      this.$post('HouseApi&a=empty_village_room_new_list', {
        ...
        layer_id: item.layer_id
		   ...
		   this.setForm('address',item.layer_name)
		this.setForm('layer_id',item.layer_id)
    ...
    clickRoom(item, idx) {
		 this.setForm('address',item.room)
		this.setForm('vacancy_id',item.pigcms_id)
      this.roomIdx = idx
      this.$router.push({
        path: '/visitor-apply',
        query: {

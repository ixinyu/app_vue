<template>
  <div class="container">
    ...
    <!-- <div class="container-box">
      <div class="h1">社区列表</div>
      <div class="card" @click="navGo">
        <div class="village-icon">
          <img src="../../assets/111.jpg" class="village-img">
        </div>
        <div class="address-box">
          <div class="title-box">
            <div class="title">同城物华天宝小区</div>
            <img src="../../assets/wancheng.png" mode="" class="renzheng">
          </div>
          <div class="distance-box">
            <div class="text">杭州拱墅区湖墅北路72号</div>
            <div class="text1">111</div>
          </div>
        </div>
      </div>
    </div> -->
    <!-- <div class="tip" v-if="pageLoading">
				<lang keywords="加载中"></lang>
			</div>

			<div class="tip" v-if="pageLoadings == true">
				<lang keywords="更多小区正在入驻,敬请期待"></lang>
			</div> -->
    <!-- <div style="text-align:center;color:#333;font-size: 28px;padding: 10px 0;" >
				<p>未搜索到相关社区</p>
			</div> -->
    <!-- 弹层 -->
    <div>
      <van-popup ref="popup" v-model="show" type="center" class="villageHousePopup">
        <div class="popup-box">
          ...
          <div class="popup-list">
            <div v-for="(item,index) in ownerList" :key="index" class="popup-card" @click="toIndex(item)">{{ item.address }}</div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$post('HouseApi&a=village_list', this.form).then(r => {
        if (this.form.keyword) {
          ...
          if (r && r.result && r.result.bind_village_list && r.result.bind_village_list.length) {
            this.list = this.list.concat(r.result.bind_village_list)
          } else {
            this.finished = true
          }
        }
    ...
    navGo(item) {
      ...
      this.item = item
      this.$post('HouseApi&a=village_list_old', {
        village_id: item.village_id
      }).then(r => {
        if (r.result.length == 1) {
          this.toIndex()
        } else if (r.result.length > 1) { // 多个户
          this.show = true
          this.ownerList = r.result
        } else {
          this.toIndex()
          // Dialog.alert({
          // 	message: '未查询到户号',
